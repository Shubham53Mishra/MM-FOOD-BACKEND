...
exports.createMealBox = async (req, res) => {
  try {
    let { title, description, minQty, price, deliveryDate, sampleAvailable, items, packagingDetails } = req.body;
    // Parse sampleAvailable to boolean
    sampleAvailable = sampleAvailable === 'true' || sampleAvailable === true;
    // Parse items to array if string
    ...
    const mealBox = await MealBox.create({
      title, description, minQty, price, deliveryDate, sampleAvailable, items, packagingDetails, boxImage, actualImage
    });
